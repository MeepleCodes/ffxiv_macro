...
import { z } from "zod";
import { Report, ReportActor, ReportFightNPC } from "./types";
...
export async function fetchReportV1(reportID: string): Promise<Report> {
  const raw = await fetch(
    `${import.meta.env.VITE_FFLOGS_V1_API}/report/fights/${reportID}?api_key=${import.meta.env.VITE_FFLOGS_V1_KEY}`
  ).then(
    resp => resp.json()
  ).then(
